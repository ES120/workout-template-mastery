...
const TemplatesPage = () => {
  ...
  return <div className="space-y-6">
      ...
      {templates.length === 0 ? <Card className="text-center py-8">
          <CardContent>
          ...
          {templates.map(template => <Card key={template.id}>
              <CardHeader>
                <div className="flex justify-between items-start py-px my-0">
                  <div>
                    <CardTitle>{template.name}</CardTitle>
                    {template.description && <CardDescription className="my-[10px]">{template.description}</CardDescription>}
                  </div>
                  <div className="flex space-x-2">
                    <Button variant="ghost" size="icon" asChild>
                      <Link to={`/new-template`} state={{
              ...
              <CardContent>
                ...
                {template.exercises.length > 0 ? <ul className="list-disc list-inside text-sm text-muted-foreground space-y-1">
                    {template.exercises.map((ex, index) => <li key={`${ex.exercise.id}-${index}`}>
                        {ex.exercise.name} ({ex.sets.length} set{ex.sets.length > 1 ? 's' : ''})
                        <ul className="list-['-_'] list-inside ml-4 text-xs">
                            {ex.sets.map((set, setIdx) => <li key={setIdx}>{set.reps} reps @ {set.weight}kg</li>)}
                        </ul>
                      </li>)}
                  </ul> : <p className="text-sm text-muted-foreground">No exercises in this template.</p>}
              </CardContent>
            </Card>)}
        </div>}
    </div>;
